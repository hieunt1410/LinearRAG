...
async def evaluate_dataset(
    ...
    results = {"context_relevancy": [], "evidence_recall": []}
    ...
    for future in asyncio.as_completed(tasks):
        try:
            ...
            completed += 1
            print(
                f"✅ Completed sample {completed}/{total_samples} - {(completed/total_samples)*100:.1f}%"
            )
    ...
    if detailed_output:
        return {"average_scores": avg_results, "detailed": detailed_results}
    else:
        return avg_results
...
async def evaluate_sample(
    ...
    return {"context_relevancy": relevance_score, "evidence_recall": recall_score}
...
async def main(args: argparse.Namespace):
    ...
    if args.mode == "API":
        ...
        ollama_embeddings = OllamaEmbeddings(
            model=args.embedding_model, base_url=args.base_url
        )
