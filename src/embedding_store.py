...
class EmbeddingStore:
    ...
    def _load_data(self):
        if os.path.exists(self.db_filename):
            ...
            print(
                f"[{self.namespace}] Loaded {len(self.hash_ids)} records from {self.db_filename}"
            )
    ...
    def _save_data(self):
        df = pd.DataFrame(
            {"hash_id": self.hash_ids, "text": self.texts, "embedding": self.embeddings}
        )
        os.makedirs(os.path.dirname(self.db_filename), exist_ok=True)
        df.to_parquet(self.db_filename, index=False)
        print(
            f"[{self.namespace}] Saved {len(self.hash_ids)} records to {self.db_filename}"
        )
